export const meta = {
  name: 'labs-auto',
  description: 'Generate 3 web-dev prototypes, judge with evolving taste, ship 1 to /apps on main',
  whenToUse: 'The hourly autonomous Labs routine. Builds 3 candidates, a taste-driven judge picks 1, ships it to /apps and pushes main (green-gated).',
  phases: [
...
const VERIFY_SCHEMA = {
  ...
  properties: {
    ...
    runDir: { type: 'string', description: 'agent/apps/runs/<ts> where screenshots live' },
    shots: {
      type: 'array',
      items: {
...
Build EXACTLY this one prototype as a finished, tasteful /apps page:
...
Three builder agents just created candidate /apps pages. Candidates (slug + files):
...
3. Once green, serve the built app and screenshot each surviving candidate:
   ...
   - For each built slug and theme in light,dark: write a small Playwright .cjs
     ...
     to agent/apps/runs/$ts/<slug>-<theme>.png. Run with the project's Playwright:
...
- Prepend a run block to agent/apps/JOURNAL.md with: timestamp ($(date -u '+%Y-%m-%d %H:%M')),
...
The judge picked winner='${verdict.winner}'. Ship EXACTLY this one to /apps on main.
...
2. ADD the winner to the live registry src/features/apps/data/apps.ts — append ONE object to the
   ...
   where sensible.
3. UPDATE memory (all in agent/apps/):
...
5. COMMIT + PUSH main: git add -A ;
   git commit -m "labs-auto: ship ${verdict.winner} to /apps" ;
