...
- winner: weather-pixels
- rationale: weather-pixels is the strongest developed frame 0 of the run AND the only still that triggers a "wait, that whole sky is voxels?" double-take — three layered value-noise fields (a pressure fbm front + a drifting cloud mask carved into dark gaps + a dawn-to-storm day-grade) drive 3,136 allocation-free GPU-instanced cubes on a domed, fogged plane, baked synchronously on mount so there is no cold start, with two distinct pokes (orbit + tap-to-summon a drifting low-pressure squall that churns a region down) and ambient lightning that flashes a whole column white. Craft 9 (cohesive domed/fogged skyscape, theme-tracked sky ramps, allocation-free per-frame instance rewrite, clean mobile framing). Novelty 9 (a SKY built from thousands of voxels driven by 3 layered value-noise fields is genuinely fresh — not a familiar silhouette). Fun 8 (orbit + tap-to-squall + ambient life = two distinct pokes plus ambient motion). Wow 9 (dense, edge-to-edge, legible rain-streak cloud bands; both themes finished). Perf/robust 8 (zero console/pageerrors, no 390px overflow, frame verified changing, single instanced draw call, reduceMotion → frozen graded still). Fit 8 (tasteful, gallery-worthy). No personalization bonus claimed — wins on dims 1–6 per P4. Verified: tap produced a visible squall dimple, drag reoriented the dome. commit-terrain (46) was the cleanest CRAFT of the three (lit heightmap, contact-shadow disc, stat strip, legend, working per-instance raycast hover readout) and carried the strongest PERSONALIZATION (+3: Monte's real 1,885 commits / peak 41 / 16d streak) — but a 3D GitHub contribution graph is a recognized genre (novelty 6) and its still is a thin diagonal ribbon adrift in a large empty ground disc, less full-frame wow than weather-pixels' dense field; the bonus closed the craft gap but could not beat a familiar concept + a sparser still. audio-bars-3d / Spectrum City (42) was technically the richest (genuine Web Audio synth: scheduled pentatonic arpeggio + kick, biquad filters, AnalyserNode FFT → per-instance tower heights; radial polar grid) and notably AVOIDS the audio cold-start trap that sank string-theory in run 3 by seeding a deterministic synthetic spectrum at frame 0, surviving the full Unmute/Beat/Ambient CTA gauntlet clean — but an FFT bar/skyline visualizer is the most familiar demo silhouette (novelty 6) and its captured stills are the weakest of the three (washed-out pale-blue bars on white in light; monochrome blue with modest height spread in dark, sparse with empty ground between towers).
- shipped: src/app/apps/weather-pixels/ (+ src/components/apps/WeatherPixels.tsx) — commit <sha>
- dropped: commit-terrain (src/app/apps/commit-terrain/, src/components/apps/CommitTerrain.tsx, src/data/commit-terrain.ts), audio-bars-3d (src/app/apps/audio-bars-3d/, src/components/apps/SpectrumCity.tsx) — files removed
