...
- Winner: weather-pixels — strongest developed frame 0 AND the only still that triggers a 'wait, that whole sky is voxels?' double-take: three layered value-noise fields (pressure fbm + drifting cloud mask + day-grade) drive 3,136 allocation-free GPU-instanced cubes on a domed/fogged plane, baked synchronously on mount (no cold start), with two distinct pokes (orbit + tap-to-summon a drifting low-pressure squall) and ambient lightning — no weak dimension.
- Losers: commit-terrain — beautiful, cleanest craft, and the strongest PERSONALIZATION (Monte's real 1,885 commits/peak/streak with verified per-instance hover readout + orbit), but a 3D contribution graph is a known genre so it lost novelty, and its still is a thin diagonal ribbon adrift in a large empty ground disc (less full-frame wow than weather-pixels' dense field). audio-bars-3d (Spectrum City) — technically the richest (genuine Web Audio synth + AnalyserNode FFT, ambient/beat modes, tap shockwaves) and notably AVOIDS the audio cold-start trap that sank string-theory in run 3 by seeding a deterministic synthetic spectrum at frame 0, plus it survives the full Unmute/Beat/Ambient CTA gauntlet clean — but an FFT bar visualizer is the most familiar demo silhouette (novelty discount), and its captured stills are the weakest of the three: washed-out pale-blue bars on white in light, monochrome blue with modest height variation in dark.
- Refinement: codify a 'still-frame TEXTURE-density' sub-rule for the Wow dimension, distinct from the existing 'is it developed / not cold-start' rule. All three pieces this run cleared the cold-start bar (every frame 0 was pre-seeded, finally) — so the new separator was which developed still also reads as DENSE and SURPRISING across the whole frame. weather-pixels fills the frame edge-to-edge with thousands of cubes whose carved cloud-bands are individually legible (high spatial detail = high screenshot value); audio-bars-3d's bars are sparse with lots of empty ground/sky between them and commit-terrain's ribbon leaves most of the disc empty. So when two+ candidates all bake a non-empty frame 0, rank the still by (a) does it fill the frame with legible, varied detail rather than a sparse subject on empty negative space, and (b) does the MEDIUM itself surprise (a sky/skyline made of unexpected primitives) — both beat 'a clean object correctly rendered.' Corollary on personalization: it remains a real but bounded bonus — commit-terrain's authentic-data hook genuinely closed the gap on craft/fit, but could not overcome a familiar concept + a sparser still, exactly as P4 intends.

### 2026-06-28-04 (run 4)
- Candidates: caustics, kaleido-forge, shatter-type
- Winner: shatter-type — the only no-weak-dimension piece, and it wins both run-3 tiebreaks: a still frame that triggers "how is that real fractured glass following the letterforms?" (from-scratch Sutherland–Hodgman Voronoi seeded only on glyph coverage, with per-shard textured blits) AND the richest second-interaction set (click any point, Shatter, swap word, Reset), plus a light MONTE / re-fuse personal nod.
- Losers: caustics — a gorgeous, genuinely physical caustic simulation (in-shader Jacobian-determinant of the refraction ray-transport map gives true caustic brightness) with a dark still as strong as shatter's, but it lost the interaction tiebreak: one gesture type (tap-to-ripple) makes it read watch-it/screensaver-y next to shatter's varied pokes. kaleido-forge — fun to draw in and a legit symmetry-group-feedback technique, but DISCOUNTED hard on the static frame for the FOURTH run running: its captured stills are faint (light: near-empty white) / hollow (dark: a sparse ring of disconnected arcs around an empty center), NOT the dense rosette its 90-pass pre-seed code claims.
- Refinement: codify a "verify the still, don't trust the seed comment" rule for warm-up/feedback pieces. Runs 1–4 every time saw a piece whose CODE promised a developed frame 0 (pre-rolled steps, 90 bloom passes, a pre-rung string) yet whose CAPTURED still was faint/hollow — and every time it cost the win. The intent in the source is not evidence; the only evidence is the actual screenshot the gallery ships. So: (a) when a candidate self-describes its first frame as "pre-seeded / pre-rolled / fully-formed," treat that as a CLAIM to falsify against the PNG, not a credit; if the still doesn't visibly match the claim, score Wow on what's shown and apply the cold-start discount regardless of how many passes the code runs. (b) New sub-tiebreak for two strong stills that both pass the wow test: prefer the one whose subject is LEGIBLE/figurative (a readable word, a recognizable form) over a purely abstract field, because a legible still is more shareable and more "I built this" — this is what nudged shatter past caustics here, and it composes with run-3's "surprise-of-technique in the still" rather than replacing it.
