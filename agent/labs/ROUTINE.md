...
judges them with the evolving taste in agent/labs/TASTE.md, and ships exactly
ONE to /labs — committing and pushing to `main` ONLY if the static build
(`pnpm build`) is green. Never push if the build is not green. If the Workflow
tool is unavailable, follow the loop documented in agent/labs/README.md using
...
- **Push to main:** the winner deploys immediately. Safe because every run is
  green-gated and each prototype is an isolated `/labs/<slug>` route.
